  ...
  return (
    <div className='flex flex-row h-30 w-full md:w-[500px] bg-yellow-50 my-2 rounded-md'>
        ...
        <div className='flex flex-col justify-center w-[60%]'>
            ...
            <p className="text-sm md:text-lg font-medium">Price: Rs. {product.price.toFixed(2)}</p>
        </div>
        <div className='flex flex-col justify-center items-center w-[20%]'>
            <p className="text-sm md:text-lg font-semibold">SubTotal</p>
            <p className="text-sm md:text-lg font-medium">Rs. {(product.price * product.quantity).toFixed(2)}</p>
        </div>
    </div>
  )
}
